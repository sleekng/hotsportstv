...
@endif
            <form class="space-y-4 mt-4" action="{{ route('hprojects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="text" name="title" value="{{ $project->title }}" placeholder="Project Title" class="w-full p-2 border border-gray-300 rounded" required>
                <input type="text" name="author" value="{{ $project->author }}" placeholder="Author" class="w-full p-2 border border-gray-300 rounded" required>

                <div class="file-input-container">
                    <label for="upload" class="file-input-label">Upload New Cover Image Here</label>
                ...
                @if ($project->image)
                    <div class="mt-4">
                        <img src="{{ asset('storage/' . $project->image) }}" alt="Current Image" class="w-48 h-auto">
                    </div>
                @endif

                <div>

                    <textarea name="content" class="w-full h-40 p-2 border border-gray-300 rounded-b" placeholder="Enter text here..." required>{{ $project->content }}</textarea>
                </div>

                <div class="flex justify-end space-x-4 mt-4">
                    <button type="submit" class="py-2 px-4 text-white bg-blue-500 rounded">Update</button>
                    <a href="{{ route('projects.index') }}" class="py-2 px-4 text-white bg-gray-500 rounded">Cancel</a>
                </div>
            </form>
        </div>
    </div>
