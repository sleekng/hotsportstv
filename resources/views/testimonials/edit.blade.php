...
@endif
            <form class="space-y-4 mt-4" action="{{ route('testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="text" name="author" value="{{ $testimonial->author }}" placeholder="Author" class="w-full p-2 border border-gray-300 rounded" required>
                ...
                @if ($testimonial->image)
                    <div class="mt-4">
                        <img src="{{ asset('storage/' . $testimonial->image) }}" alt="Current Image" class="w-48 h-auto">
                    </div>
                @endif


                <textarea name="content" placeholder="Testimonial Content" class="w-full p-2 border border-gray-300 rounded" required>{{ $testimonial->content }}</textarea>
                <div class="flex justify-start space-x-4 mt-4">
                    <button type="submit" class="py-2 px-4 text-white bg-blue-500 rounded">Update</button>
                    <a href="{{ route('testimonials.index') }}" class="py-2 px-4 text-white bg-gray-500 rounded">Cancel</a>
                </div>
            </form>
        </div>
    </div>
