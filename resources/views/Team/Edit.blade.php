...
@endif
            <form class="space-y-4 mt-4" action="{{ route('team.update', $team->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="text" name="name" value="{{ $team->name }}" placeholder="Team Member's Name" class="w-full p-2 border border-gray-300 rounded" required>
                <input type="text" name="role" value="{{ $team->role }}" placeholder="Role / Designation" class="w-full p-2 border border-gray-300 rounded" required>
               
                
                <div class="file-input-container">
                    <label for="upload" class="file-input-label">Upload New Team Image</label>
                ...
                @if ($team->image)
                    <div class="mt-4">
                        <img src="{{ asset('storage/' . $team->image) }}" alt="Current Image" class="w-48 h-auto">
                    </div>
                @endif
                ...
                <div class="flex justify-start space-x-4 mt-4">
                    ...
                    <a href="{{ route('team.index') }}" class="py-2 px-4 text-white bg-gray-500 rounded">Cancel</a>
                </div>
            </form>
        </div>
    </div>
