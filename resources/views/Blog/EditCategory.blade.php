...
@endif
            ...
            <form action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')
                <input type="text" name="name" value="{{ $category->name }}" placeholder="Category Name" class="w-full p-2 border border-gray-300 rounded" required>
    
                <div class="file-input-container">
                    <label for="upload" class="file-input-label">Upload Category Image</label>
                ...
                @if ($category->image)
                    <div class="mt-4">
                        <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-48 h-auto">
                    </div>
                @endif
